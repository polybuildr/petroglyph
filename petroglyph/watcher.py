...
class Watcher(object):
    ...
    class Meta(object):
        """Meta class to store metadata of posts."""
        ...
        def repr_tuple(self):
            """Returns a tuple representation of the Meta object."""
    ...
    def _file_hash(self, f, block_size=2**20):
        """Generates an MD5 hash of a given file."""
        md5 = hashlib.md5()
        while True:
    ...
    def update_posts_meta(self, new_meta = None):
        """Writes generated metadata into the meta file."""
    ...
    def _read_posts_meta_file(self):
        """Reads the posts.csv file containing metadata and returns it as
    ...
    def generate_posts_meta(self, write = False):
        """Generates an array of Meta objects for currently existing posts
    ...
    def get_posts_meta(self, only_changed = False):
        ...
        for meta_item in new_meta:
            mtime = meta_item.mtime
            if mtime.year not in archive:
                archive[mtime.year] = {}
            if mtime.month not in archive[mtime.year]:
                archive[mtime.year][mtime.month] = {}
            if mtime.day not in archive[mtime.year][mtime.month]:
                archive[mtime.year][mtime.month][mtime.day] = []
            archive[mtime.year][mtime.month][mtime.day].append(meta_item.slug)
